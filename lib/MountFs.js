...
var MountFs = module.exports = function MountFs(options) {
    ...
    var pathByOpenFileDescriptorNumber = {};
        ...
        if (typeof fsPropertyValue === 'function' && /^[a-z]/.test(fsMethodName)) {
            that[fsMethodName] = function (firstArg) { // ...
                ...
                if (typeof firstArg === 'number') {
                    path = pathByOpenFileDescriptorNumber[firstArg];
                    if (!path) {
                        throw new Error('MountFs: fd ' + firstArg + ' does not appear to be open');
                    }
                ...
                if (/(?:Sync|Stream)$/.test(fsMethodName)) {
                    ...
                    try {
                        ...
                        if (fsMethodName === 'openSync' && typeof result === 'number') {
                            pathByOpenFileDescriptorNumber[result] = firstArg;
                        } else if (fsMethodName === 'closeSync' && typeof args[0] === 'number') {
                            delete pathByOpenFileDescriptorNumber[args[0]];
                        }
                        return result;
                    } catch (err) {
                    ...
                    args.push(function (err, result) {
                        if (fsMethodName === 'readdir') {
                            ...
                            pathByOpenFileDescriptorNumber[result] = firstArg;
                        } else if (fsMethodName === 'close' && typeof args[0] === 'number') {
                            delete pathByOpenFileDescriptorNumber[args[0]];
                        }
